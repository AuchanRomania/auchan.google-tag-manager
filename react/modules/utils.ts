...
export function formatCartItemsAndValue(
  ...
  options?: {
    ...
    /** @deprecated use useStoredListAttribution */
  ...
  const useStoredListAttribution =
    options?.useStoredListAttribution !== false &&
    options?.useListAttribution !== false
  ...
  const items = cartItems.map((item: CartItem) => {
    ...
    const formattedListPrice =
      listRaw != null
        ? toMajorCurrency(
            listRaw,
            item.originalPrice != null || hasListPrice
              ? Boolean(item.priceIsInt ?? options?.dividePrice ?? true)
              : Boolean(shouldFormatPrice)
          )
        : formattedPrice

    const discountFromFields =
      item.discount !== undefined
        ? Math.max(
            0,
            sellingIsInt || shouldFormatPrice
              ? item.discount / 100
              : item.discount
          )
        : Math.max(0, formattedListPrice - formattedPrice)

    const discount = Math.round(discountFromFields * 100) / 100
    ...
    const pixelListName = item.item_list_name || item.list
    const pixelListId = item.item_list_id
    const pixelPosition = item.index
